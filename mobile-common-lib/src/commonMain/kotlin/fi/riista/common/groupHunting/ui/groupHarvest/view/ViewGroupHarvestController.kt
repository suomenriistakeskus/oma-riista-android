...
import fi.riista.common.groupHunting.ui.HarvestActionResolver
...
class ViewGroupHarvestController(
    ...
    private fun createViewModel(harvestData: GroupHuntingHarvestData,
                                canEditHarvest: Boolean,
                                canApproveHarvest: Boolean,
                                canRejectHarvest: Boolean): ViewGroupHarvestViewModel {
        ...
        dataFieldProducers = GroupHarvestField.values().mapNotNull { field ->
            when (field) {
                GroupHarvestField.LOCATION -> field to { harvest: GroupHuntingHarvestData ->
                    LocationField(field, harvest.geoLocation) {
                        readOnly = true
                    }
                }
                GroupHarvestField.SPECIES_CODE -> field to { harvest: GroupHuntingHarvestData ->
                    SpeciesCodeField(field, harvest.gameSpeciesCode) {
                        readOnly = true
                    }
                }
