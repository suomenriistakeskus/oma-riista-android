...
internal class ModifyGroupHarvestEventToIntentMapper(
    ...
    override val stringWithIdEventDispatcher = StringWithIdEventDispatcher<GroupHarvestField> { fieldId, newValue ->
        val intent = when (fieldId) {
            GroupHarvestField.DEER_HUNTING_TYPE -> ModifyGroupHarvestIntent.ChangeDeerHuntingType(newValue.toBackendEnum())
            GroupHarvestField.ANTLERS_TYPE -> ModifyGroupHarvestIntent.ChangeAntlersType(newValue.toBackendEnum())
            GroupHarvestField.FITNESS_CLASS -> ModifyGroupHarvestIntent.ChangeFitnessClass(newValue.toBackendEnum())
            GroupHarvestField.ACTOR -> ModifyGroupHarvestIntent.ChangeActor(newValue)
