...
internal class AddShootingTestParticipant(
    val shootingTestEventId: ShootingTestEventId,
    val shootingTestParticipant: ShootingTestParticipantCreateDTO,
) : NetworkRequest<Unit> {

    override suspend fun request(client: NetworkClient): NetworkResponse<Unit> {
        val url = "${client.serverBaseAddress}/api/mobile/v2/shootingtest/event/$shootingTestEventId/participant"

        val payload = shootingTestParticipant.serializeToJson()
        requireNotNull(payload) {
            "Failed to serialize shooting test participant data to json"
        }
