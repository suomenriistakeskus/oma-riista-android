...
class GroupHuntingDiaryProviderTest {
    ...
    fun testObservationHasCorrectValues() {
        ...
        assertEquals(4, obs.observationSpecVersion)
        assertEquals(1, obs.specimens.size)
        val specimen = obs.specimens[0]
        assertEquals(20, specimen.id)
        assertEquals(0, specimen.rev)
        assertEquals(Gender.FEMALE, specimen.gender?.value)
        assertEquals(ObservedGameAge.UNKNOWN, specimen.age?.value)
        assertEquals(ObservedGameState.WOUNDED, specimen.state?.value)
        assertEquals(GameMarking.EARMARK, specimen.marking?.value)
